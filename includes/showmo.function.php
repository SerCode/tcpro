...
function showMonth($year,$month,$groupfilter,$sortorder,$page=1) {
   ...
   require_once( "includes/tctpl.class.php" );
   ...
   $A  = new tcAbs;
   $AC = new tcAbs; // for Absence Count Array
                     ...
                     foreach ($absences as $abs) {
                        ...
                        if ($abs['show_in_remainder']) {
                           if (isAllowed("editAllUserCalendars") OR isAllowed("editGroupUserCalendars") OR isAllowed("editOwnUserCalendars")) {
                              if ( $AL->findAllowance($U->username,$abs['id']) ) {
                              ...
                              $thisYearTaken     = countAbs(addslashes($U->username),$abs['id'],$from,$to);
